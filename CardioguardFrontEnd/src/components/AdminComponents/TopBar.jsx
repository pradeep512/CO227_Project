...
const getRandomGrey = () => {
  const greys = [
    "#d3d3d3", "#a9a9a9", "#808080", "#696969", "#505050"
  ];
  return greys[Math.floor(Math.random() * greys.length)];
};

const TopBar = ({ patientName, patientID, homeLink, logoutLink }) => {
  ...
  return (
    <div className="bg-white p-4 shadow-md flex justify-between items-center" style={{ boxShadow: "0 4px 8px rgba(0, 0, 0, 0.2)" }}>
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="flex items-center">
          ...
          <p className="ml-3 text-lg font-semibold">{patientName || "UserName"}</p>
        </div>
      </div>
    </div>
  );
